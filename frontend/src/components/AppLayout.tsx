...
export default function AppLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <Box sx={{ display: 'flex', height: '100vh', overflow: 'hidden', bgcolor: 'background.default' }}>
      ...
      {isMobile && (
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', px: 1.5, py: 1, minHeight: 56 }}>
            ...
            <Box
              sx={{ display: 'flex', alignItems: 'center', gap: 1, cursor: 'pointer', flex: 1, justifyContent: 'center' }}
              onClick={() => navigate('/')}
            >
              <Typography sx={{ fontWeight: 700, fontSize: '1rem', background: `linear-gradient(135deg, ${BRAND.cyan} 0%, ${BRAND.purpleLight} 100%)`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                Skillhub
              </Typography>
            </Box>
            <Box sx={{ width: 40 }} />
      ...
      {/* ── Sidebar (temporary no mobile, permanent no desktop) ── */}
      <Drawer
      ...
      >
        <Box sx={{ flex: 1, minHeight: 0, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'auto', overflowX: 'hidden' }}>
          <Box sx={{ pl: { xs: 1.5, sm: 2, md: 3 }, pr: { xs: 1.5, sm: 2, md: 3 }, flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
            {children}
          </Box>
        </Box>
      </Box>
